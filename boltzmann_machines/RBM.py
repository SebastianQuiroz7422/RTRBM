...
if __name__=='__main__':
    ...
    n_visible, n_hidden, T = 16, 3, 64
    data = create_BB(n_visible=n_visible, T=T, n_samples=24, width_vec=[4, 5, 6], velocity_vec=[2, 3], boundary=False)
    # mean = torch.zeros(n_hidden, 20)
    # std = torch.zeros(n_hidden, 20)
    # [train, test] = torch.load(r'C:\Users\sebas\Desktop\Figure 2\train_test_list')

    rbm = RBM(data, n_hidden=3, device="cpu", debug_mode=True)
    rbm.learn(batch_size=100, n_epochs=2, lr=1e-3, CDk=15, mom=0.6, wc=0.0002, sp=0, x=0)
    torch.save(rbm, r'C:\Users\sebas\Desktop\Figure 2\rbm')

    vt_infer, rt_infer = rbm.sample(torch.tensor(data[:, 0, :], dtype=torch.float))
    ...
    n_visible, T, num_samples = data.shape
    data1 = torch.zeros(n_visible, T * num_samples)
